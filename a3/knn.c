...
int knn_predict(Dataset *data, Image *input, int K, double (*fptr)(Image *, Image *)) {
    ...
    for (int i = 0; i < data->num_items; i++) {
        ...
        // Change the call below to call distance function passed in as a parameter (Edited)
...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    if(write(p_out, &result, sizeof(int)) == -1){
        perror("write");
    }
...
double distance_cosine(Image *a, Image *b){
    ...
    d = (2/M_PI)*acos(((sum_ab)/(sqrt(sum_a_sqr)*sum_b_sqr)));
    return d;
